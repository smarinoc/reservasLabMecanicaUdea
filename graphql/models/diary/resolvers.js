...
const DiaryResolvers = {
    ...
    Diary: {
        schedules: async (parent) => await prisma.schedule.findMany({
            where: {
                diaryId: parent.id
            }
        })
    ...
    Schedule: {
        diary: async (parent) => await prisma.diary.findUnique({
            where: {
                id: parent.diaryId
            }
        }),
    ...
    Query: {
        ...
        getScheduleAvailable: async ()=> {
            ...
            const result = schedules.reduce((acc,item)=>{
                if(!acc.find((element)=>item.day===element.day && item.hour===element.hour)){
                    acc.push(item);
                }
                return acc;
              },[])

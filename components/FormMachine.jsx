...
import InputMachineUnit from './InputMachineUnit';
...
const FormMachine = ({ machine, onSubmit, onDelete, loading }) => {
    ...
    const [recommendations, setRecommendations] = useState(machine?.recommendations || [])
    const [image, setImage] = useState(machine?.image || [])
    const [machineUnits, setMachineUnits] = useState(machine?.machineUnits.map((item) => ({ location: item.location, count: item.count })) || [{
        location: "",
        count: 0
    }])
    ...
    const onUploadImage = async () => {
        ...
        try {
            const res = await fetch("https://api.cloudinary.com/v1_1/daef66ohy/image/upload", {
                method: 'POST',
                body: formData
            });
                ...
                <div className='w-full flex flex-col gap-1'>
                    ...
                    <ul className={recommendations.length > 0 ? 'border-2' : ''}>
                        {
                            recommendations.map((item, index) =>
                                <li className='flex flex-row items-center w-full pr-8 my-2'>
                                    ...
                                    <div className='text-black px-3 font-medium whitespace-normal w-full' key={index}>{item}</div>
                                    <BsXlgButton onClick={() => {
                    ...
                    machineUnits.map((item, index) =>
                        <div className='w-full'>
                            <InputMachineUnit unit={item} onCancelMachineUnit={onCancelMachineUnit} index={index} />
                        </div>
                    )
                }
                <Button text="Añadir otra ubicación" onClick={addLocation} />
            </div>
            <div className={`flex w-full ${onDelete? "flex-row justify-around": "justify-end"}`}>
                <Button text={machine ? "Editar máquina" : "Crear máquina"} disabled={disabledButton} onClick={async () => {
                ...
                {
                    onDelete? 
                    <Button text="Eliminar máquina" onClick={()=>onDelete(machine)} disabled={disabledButton} />
                    : <></>
                }
            </div>
        </div>
